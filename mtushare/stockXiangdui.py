...
def getDf(stock_code,starttime, endtime):
    ...
    data = pd.read_csv('./info/%s.csv' % stock_code)
    data.datetime = pd.to_datetime(data['datetime'])
    data = data.set_index('datetime',drop=False)
    dfz = data[endtime:starttime]
...
def drawcand(fig,stock,name,df):
    global updateflag
    ...
    if len(stock.closes)<30:
        total = 9
    elif len(stock.closes)>30 and len(stock.closes)<90:
        total = 30
    elif len(stock.closes)>90 and len(stock.closes)<180:
        total = 60
    elif len(stock.closes)>100:
        total = 66

    print('----1----')
    print(total)
    print('----1----')
    num = int(len(stock.data_index)/total)
    ax.set_xlim(0, total)
    ...
    for i in range(0,len(stock.data_index),num):
        index.append(i)
    print(index)
    priceList = [stock.opens[index] for index in index]
    priceList.reverse()
    dateList = [str(stock.data_index[index]) for index in index]
    # if len(dateList)>9:
    #     remain = int(len(dateList)/9)
    ...
    for i in range(0,len(priceList)):
        percentList.append(priceList[i]/priceList[0])

    def to_percent(temp, position):
        return '%1.0f' % (100 * temp-100) + '%'
    ...
    print('----------')
    print(name)
    plt.plot(dateList,percentList,label=name)
    ...
    # stock.data_index.columns = ['datetime']
    # print(stock.data_index)
    # print(type(stock.data_index[0]))
    # print(stock.data_index[0])
    # print(stock.opens)
    # print(stock.opens)
    # stock.data_index.datetime = pd.to_datetime(stock.data_index['datetime'])
    # print(stock.data_index.columns)
    # print(stock.data_index[datetime])
    # x=df['datetime']
    # y=df['close']
    # print(x)
    # print(y)
    # plt.plot(x, y, color="deeppink", linewidth=2, linestyle=':', label='Jay income', marker='+')
    # plt.show()
    # data = pd.DataFrame(columns=['datetime'],data=stock.data_index)
    # price = pd.DataFrame(columns=['close'],data=stock.closes)
    # data.datetime = pd.to_datetime(data['datetime'])
    #
    # print(data)
    ...
    # print(price['close'])
    # print(price)
    # print(stock.closes.columns)
    # price_new = price.astype(np.int32)
    #
    #
    # plt.plot(data['datetime'],price['close'],color="deeppink",linewidth=2,linestyle=':',label='Jay income', marker='o')
    # if updateflag==0:
    ...
    # else:
    #     x_data = ['2011', '2012', '2013', '2014', '2015', '2016', '2017']
    #     y_data = [48000, 30200, 63000, 71000, 84000, 20500, 207000]
    #
    #     plt.plot(x_data, y_data)
    # print('---------')
    # print(updateflag)
...
def get_code(root,start_time,end_time):
    ...
    for codename in stockList:
        ...
        stock_code = codename
        name = stock_code
        df = getDf(stock_code, starttime, endtime)
        stock = stockInfo(days, df)
        figure = drawcand(figure, stock, name,df)  # 返回matplotlib所画图形的figure对象
